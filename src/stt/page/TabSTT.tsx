...
export default function TabSTT() {
  ...
  const updateSttState = (
    ...
    setStts((prevStts) =>
      prevStts.map((stt) => (stt.id === sttId ? { ...stt, ...newProps } : stt))
    );
  };
  ...
  useEffect(() => {
    ...
    const fetch = async () => {
      try {
        ...
        //업로드 화면 or 결과 화면
        if (response.length !== 0) setSelectedSttId(response[0].id);
        ...
        if (apiError.status === 404) {
          ...
          alert(response ?? "STT 불러오기 실패");
          console.error("STT 불러오기 실패:", apiError);
        }
      }
    };
  ...
  const validateFile = (file: File): boolean => {
    ...
    if (sizeMB > maxFileSizeMB) {
      alert(
        `파일 크기가 ${maxFileSizeMB}MB를 초과했습니다. (현재: ${sizeMB.toFixed(
          2
        )}MB)`
      );
      return false;
  ...
  const handleUploadFile = async (file: File) => {
    ...
    try {
      ...
      const apiError = error as ApiError;
      const response = apiError.response?.data?.message;
      alert(response ?? "음성 파일 등록 중 오류가 발생했습니다.");
  ...
  const handleDelete = async (sttId: number) => {
    ...
    try {
      ...
      const apiError = error as ApiError;
      const response = apiError.response?.data?.message;
      alert(response ?? "stt 삭제 중 오류가 발생했습니다.");
    }
  };
  ...
  const handleSummarySave = async () => {
    ...
    if (!currentStt?.isEditable) return;
    ...
    try {
      ...
      const apiError = error as ApiError;
      const response = apiError.response?.data?.message;
      alert(response ?? "STT 저장 중 오류가 발생했습니다.");
    }
  };
      ...
      const startChunkTimer = () => {
        recordTimeTimerRef.current[sttId] = setInterval(() => {
          setStts((prevStts) =>
            prevStts.map((stt) =>
              stt.id === sttId
                ? { ...stt, recordingTime: (stt.recordingTime || 0) + 1 }
                : stt
            )
          );
        }, 1000);
        ...
        {findSttById(selectedSttId)?.isTemp
          ? (() => {
              ...
              if (
                ...
                currentStt?.recordingStatus === "paused"
              ) {
                return (
                  <Box
                    sx={{
                    ...
                    <Box sx={{ display: "flex", gap: 2 }}>
                      {currentStt.recordingStatus === "recording" ? (
                        <Tooltip title="일시정지">
                          <IconButton
                            size="large"
                            onClick={() => handlePauseRecording(selectedSttId)}
                          >
                        ...
                        <Tooltip title="다시 시작">
                          <IconButton
                            size="large"
                            onClick={() => handleResumeRecording(selectedSttId)}
                          >
                      ...
                      <Tooltip title="종료">
                        <IconButton
                          size="large"
                          color="error"
                          onClick={() => handleStopRecording(selectedSttId)}
                        >
                  ...
                  >
                    ...
                    >
                      ...
                      <Button
                        variant="outlined"
                        color="secondary"
                        onClick={() => handleRecordAgain(selectedSttId)}
                      >
                        다시 녹음하기
                      </Button>
                    </Box>
                    ...
                    <Box sx={{ textAlign: "center" }}>
                      <Tooltip title="즉시 녹음 시작">
                        <IconButton
                          ...
                          onClick={() => handleStartRecording(selectedSttId)}
                        >
          ...
          : stts.length !== 0 && (
              <Box>
                <Box
                  sx={{ display: "flex", gap: 2, alignItems: "start", mt: 3 }}
                >
                  <Box sx={{ flex: 1 }}>
                    <Typography>
                      요약 결과
                    ...
                    <TextField
                      ...
                      sx={{
                        ...
                        "& .MuiOutlinedInput-root": {
                          borderRadius: 1.5,
                          bgcolor: findSttById(selectedSttId)?.isLoading
                            ? "#f0f0f0"
                            : "#fafafa",
                        },
